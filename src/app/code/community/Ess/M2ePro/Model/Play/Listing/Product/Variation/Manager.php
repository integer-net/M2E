...
class Ess_M2ePro_Model_Play_Listing_Product_Variation_Manager
{
    // ########################################
    ...
    {
        return (bool)(int)$this->getPlayListingProduct()->getData('is_variation_product');
    }

    public function isVariationProductMatched()
    {
        return (bool)(int)$this->getPlayListingProduct()->getData('is_variation_product_matched');
    }
    ...
    private function createStructure(array $options)
    {
        $variationId = Mage::helper('M2ePro/Component_Play')
    ...
    {
        $items = Mage::getModel('M2ePro/Play_Item')->getCollection()
                    ...
                    ->addFieldToFilter('sku',$this->getPlayListingProduct()->getSku())
        ...
        foreach ($items as $item) {
            /* @var $item Ess_M2ePro_Model_Play_Item */
            $item->deleteInstance();
        }
    }
    ...
    private function createChannelItem(array $options)
    {
        ...
        Mage::getModel('M2ePro/Play_Item')->setData($data)->save();
    }
