...
class Ess_M2ePro_Model_Play_Search_Settings
{
    ...
    public function process()
    {
        ...
        if (!$this->setNextStep()) {
            $this->getListingProduct()->setData(
                'search_settings_status', Ess_M2ePro_Model_Play_Listing_Product::SEARCH_SETTINGS_STATUS_NOT_FOUND
            );
        ...
        $result = Mage::getModel('M2ePro/Connector_Play_Dispatcher')->processConnector(
            'settings', $this->getSearchMethod(), 'requester', $this->getConnectorParams(),
            $this->getListingProduct()->getAccount(), 'Ess_M2ePro_Model_Play_Search'
        );
    ...
    private function processResult()
    {
        ...
        $type = 'string';
        if ($this->step != self::STEP_MAGENTO_TITLE) {
            $type = Mage::helper('M2ePro')->isISBN($params['query']) ? 'isbn' : 'ean';
        }
        ...
        if ($this->canPutResultToSuggestData($result)) {
            ...
            $this->getListingProduct()->setData(
                ...
                Ess_M2ePro_Model_Play_Listing_Product::SEARCH_SETTINGS_STATUS_ACTION_REQUIRED
            );
    ...
    private function getGeneralIdFromResult($result)
    {
        ...
        return key($result[0]['variations']['play_ids']);
    }
    ...
    private function canPutResultToSuggestData($result)
    {
        ...
        if (isset($result[0]['variations']) && count($result[0]['variations']['play_ids']) != 1) {
            return true;
        }
    ...
    private function getQueryParam()
    {
        switch ($this->step) {
            case self::STEP_GENERAL_ID:
                ...
                $query = $this->getPlayListingProduct()->getGeneralId();
                empty($query) && $query = $this->getPlayListingProduct()->getListingSource()->getSearchGeneralId();
            ...
            case self::STEP_MAGENTO_TITLE:
                ...
                if ($this->getPlayListingProduct()->getPlayListing()->isSearchByMagentoTitleModeEnabled()) {
                    $query = $this->getPlayListingProduct()->getActualMagentoProduct()->getName();
                }
    ...
    private function getSearchMethod()
    {
        if ($this->step == self::STEP_GENERAL_ID) {
            if ($this->getPlayListingProduct()->getPlayListing()->isGeneralIdWorldwideMode() ||
                $this->getPlayListingProduct()->getPlayListing()->isGeneralIdIsbnMode()
            ) {
                return 'byEanIsbn';
            }
        }
    ...
    private function filterReceivedItemsFullTitleMatch($results)
    {
        ...
        $magentoProductTitle = $this->getPlayListingProduct()->getActualMagentoProduct()->getName();
