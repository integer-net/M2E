...
class Ess_M2ePro_Block_Adminhtml_Play_Listing_ProductSearch_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    private $productId;
    ...
    {
        $categoryCode = $row->getData('category_code');

        if (empty($value)) {
            $url = Mage::helper('M2ePro/Component_Play')->getItemUrl('%general_id%',$categoryCode);
        ...
        $url = Mage::helper('M2ePro/Component_Play')->getItemUrl($value, $categoryCode);
    ...
    public function callbackColumnTitle($value, $row, $column, $isExport)
    {
        ...
        foreach ($variations['set'] as $specificName => $specific) {
            ...
            $specificsHtml .= '<select class="specifics_'.$id.'"
                                       onchange="PlayListingProductSearchHandlerObj.specificsChange(this)"
    ...
    public function callbackColumnActions($value, $row, $column, $isExport)
    {
        ...
        if (!is_null($row->getData('variations'))) {
            $templateMapHtml = '<a href="javascript:void(0);" onclick="PlayListingProductSearchHandlerObj.mapToGeneralId('
                .$this->productId
        ...
        return '<a href="javascript:void(0);" onclick="PlayListingProductSearchHandlerObj.mapToGeneralId('
            .$this->productId
            .', \''
...
JAVASCRIPT;
    ...
    {
        return $this->getUrl('*/adminhtml_play_listing/getSuggestedPlayIDGrid', array('_current'=>true));
    }
